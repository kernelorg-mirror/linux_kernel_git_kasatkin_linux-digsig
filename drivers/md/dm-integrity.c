			      ...
			      u8 *collected, int update)
{
	...
	pr_debug("hmac: block: %llu, record: %u\n", (loff_t)block, index);
	...
	if (!update) {
		...
		if (err) {
			...
			if (dmi->flags & DM_INT_FLAGS_VERBOSE) {
				DMERR("hmac mismatch: block: %llu, record: %u",
				      (loff_t)block, index);
...
static void dm_int_verify_io(struct dm_int_io *io)
{
	...
	loff_t offset = io->bi_iter.bi_sector << SECTOR_SHIFT;
	...
	pr_debug("block: %llu, size: %u\n",
		 (loff_t)io_block(io), io->bi_iter.bi_size);

	while (io->bi_iter.bi_size) {
		bv[0] = bio_iter_iovec(bio, io->bi_iter);
		pr_debug("bv: offset: %llu, bv_offset: %u, bv_len: %u, idx: %u\n",
			 offset, bv->bv_offset, bv->bv_len, io->bi_iter.bi_idx);
		...
		offset += bv->bv_len;
		bio_advance_iter(bio, &io->bi_iter, bv->bv_len);
	}

	WARN(!err && io->bi_iter.bi_size,
	     "bio has date left: %d\n", io->bi_iter.bi_size);
...
static void dm_int_end_io(struct bio *bio, int err)
{
	...
	pr_debug("pending: %d, block: %llu\n",
		 atomic_read(&io->count), (loff_t)io_block(io));

	if (unlikely(!bio_flagged(bio, BIO_UPTODATE) && !err))
		err = -EIO;

	if (err || bio->bi_iter.bi_size)
		DMERR("bio failed: %d, left: %u",
		      err, bio->bi_iter.bi_size);
...
static int dm_int_map(struct dm_target *ti, struct bio *bio)
{
	...
	bio->bi_iter.bi_sector = dmi->start +
	...
	if (!bio->bi_iter.bi_size)
		DMERR("bio without data: size: %d", bio->bi_iter.bi_size);

	BUG_ON(bio->bi_iter.bi_sector & (to_sector(dmi->data_block_size) - 1));
	BUG_ON(bio->bi_iter.bi_size & (dmi->data_block_size - 1));
	...
	pr_debug("%s block: %llu, size: %u, %s (%d)\n",
		 bio_data_dir(bio) ? "write" : "read", (loff_t)io_block(io),
		 io->bi_iter.bi_size, current->comm, current->pid);
...
static int dm_int_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	...
	dmi->data_block_bits = __ffs(dmi->data_block_size);
	...
	dmi->hmac_block_bits = __ffs(dmi->hmac_block_size);
	...
	if (is_power_of_2(dmi->hmac_size)) {
		dmi->hmac_block_shift =
			dmi->hmac_block_bits - __ffs(dmi->hmac_size);
